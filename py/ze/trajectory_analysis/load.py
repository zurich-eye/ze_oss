...
def load_dataset_csv(results_dir):
    ...
    matches = np.genfromtxt(os.path.join(results_dir, 'groundtruth_matches.csv'), dtype=np.longlong, delimiter=',', skip_header=1)
    matches = dict([(row[0], row[1]) for row in matches]) # { es -> gt }
    
    keys_es = read_nanosecond_timestamps_from_csv_file(filename_es, col=0, delimiter=' ')
    data_es = np.genfromtxt(filename_es, delimiter=' ', dtype=np.float64, skip_header=1)[:,1:]
    
    keys_gt = np.genfromtxt(filename_gt, usecols=(0), delimiter=',', dtype=np.longlong, skip_header=1)
    ...
    for i in range(len(keys_es)):
        if keys_es[i] in matches:
            ...
            tmp = gt_data[3:7] # quaternion order we want is x y z w, file has x y z w
